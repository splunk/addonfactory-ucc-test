...
class Configuration(ConfigurationBase):
    ...
    def set_up(self, api_instance):
        ...
        try:
            api_response = (
                api_instance.demo_addon_for_splunk_settings_logging_post(
                    output_mode, loglevel="DEBUG"
                )
            )
            pprint(api_response)
        except ApiException as e:
            print(
                "Exception when calling DefaultApi->demo_addon_for_splunk_settings_logging_post: %s\n"
                % e
            )
        ...
        for endpoint_name, endpoint_uri in self.endpoints.items():
            try:
                api_response = (
                    api_instance.demo_addon_for_splunk_endpoint_post(
                        output_mode, name=endpoint_name, uri=endpoint_uri
                    )
                )
                pprint(api_response)
            except ApiException as e:
                print(
                    "Exception when calling DefaultApi->demo_addon_for_splunk_endpoint_post: %s\n"
                    % e
                )
        ...
        for input_configuration in self.get_all_inputs():
            if isinstance(input_configuration, InputTypeEndpointConfiguration):
                try:
                    api_response = (
                        api_instance.demo_addon_for_splunk_demo_input_post(
                            output_mode,
                            name=input_configuration.name,
                            interval=input_configuration.interval,
                            endpoint=input_configuration.endpoint_name,
                            sourcetype=input_configuration.source_type,
                    ...
                    print(
                        "Exception when calling DefaultApi->demo_addon_for_splunk_demo_input_post: %s\n"
                        % e
                    )
    ...
    def tear_down(self, api_instance):
        ...
        for input_configuration in self.get_all_inputs():
            if isinstance(input_configuration, InputTypeEndpointConfiguration):
                try:
                    api_response = api_instance.demo_addon_for_splunk_demo_input_name_post(
                        output_mode, input_configuration.name, disabled=True
                    )
                    pprint(api_response)
                except ApiException as e:
                    print(
                        "Exception when calling DefaultApi->demo_addon_for_splunk_demo_input_name_post: %s\n"
                        % e
                    )
