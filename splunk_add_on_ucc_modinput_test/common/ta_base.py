...
from tests.modinput.vendor_product import Configuration as VendorProductConfiguration
...
class ConfigurationBase():
    ...
    def __init__(self, *, splunk_configuration: SplunkConfiguration, vendor_product_configuration: VendorProductConfiguration):
        self.splunk_configuration = splunk_configuration
        self.vendor_product_configuration = vendor_product_configuration

        configuration = swagger_client.Configuration()
        configuration.host = configuration.host.replace('{domain}', self.splunk_configuration.host)
        configuration.host = configuration.host.replace('{port}', self.splunk_configuration.port)

        configuration.verify_ssl = False
        configuration.username = self.splunk_configuration.username
        configuration.password = self.splunk_configuration.password

        self._api_instance = swagger_client.DefaultApi(swagger_client.ApiClient(configuration))
        self._inputs = {}
    ...
    def add_input_configuration(self,input_configuration: InputConfigurationBase):
    ...
    def get_input_configuration(self,name_prefix: str) -> InputConfigurationBase:
